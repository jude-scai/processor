...
class UnderwritingScheduler:
    ...
    def __init__(self, max_workers: int = 3):
        ...
        self._queue_locks: Dict[str, threading.Lock] = (
            {}
        )  # underwriting_id -> queue_lock
        self._manager_lock = threading.Lock()  # Protects the dictionaries
        self._executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="underwriting-scheduler"
        )
    ...
    def _start_scheduler_worker(self):
        ...
        worker_thread = threading.Thread(
            target=worker, daemon=True, name="scheduler-worker"
        )
        worker_thread.start()

    def schedule_workflow(
        self, underwriting_id: str, workflow_func: Callable, *args, **kwargs
    ) -> str:
        """
        ...
        print(
            f"    📋 Queued work item {work_item_id} for underwriting {underwriting_id}"
        )
        return work_item_id
    ...
    def _process_work_item(self, work_item: Dict[str, Any]):
        ...
        print(
            f"    🚀 Processing work item {work_item_id} for underwriting {underwriting_id}"
        )
        ...
        try:
            ...
            print(
                f"    ✅ Completed work item {work_item_id} for underwriting {underwriting_id}"
            )
            ...
            print(
                f"    ❌ Failed work item {work_item_id} for underwriting {underwriting_id}: {e}"
            )
    ...
    def clear_queue(self, underwriting_id: str) -> int:
        ...
        with self._manager_lock:
            ...
            print(
                f"    🧹 Cleared {cleared_count} items from queue for underwriting {underwriting_id}"
            )
